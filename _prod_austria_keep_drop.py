"""Run the strict keep/drop gate over the existing Austria campaign.
...
Usage:
    python _prod_austria_keep_drop.py            # inspect only
    python _prod_austria_keep_drop.py --start    # enqueue the pass
    python _prod_austria_keep_drop.py --watch    # poll progress
"""
...
def main() -> int:
    ...
    if do_start:
        ...
        result = req(
            "POST",
            f"/maps/runs/{AUSTRIA_RUN_ID}/keep-drop",
            token=token,
            org_id=org_id,
        )
