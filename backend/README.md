...
Chat composer uploads (`.txt`, `.md`, `.csv`, `.json`, `.xml`, `.yaml`, `.html`, `.docx`, `.xlsx`, `.pdf`, `.png`, `.jpg`, `.jpeg`, `.webp`, …) are streamed into `CHAT_ATTACHMENT_DIR` with a hard size limit during upload. DOCX/XLSX/PDF are parsed server-side for text excerpts (no OCR). Images are stored as binary; before the council runs, a single dedicated vision model (`CHAT_IMAGE_ANALYSIS_MODEL`, default `gemini`) produces structured IMAGE CONTEXT text that is supplied to every council model and the verdict. Docker Compose mounts a named `chat_attachments` volume at `/app/data/chat_attachments` on the API service only so container recreation keeps files. Production deployments must persist that directory; reverse proxies must allow request bodies at least as large as `CHAT_ATTACHMENT_MAX_BYTES`.
