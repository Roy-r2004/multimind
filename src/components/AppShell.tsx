...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative flex min-h-screen w-full text-foreground">
      ...
      <header className="fixed top-0 left-0 right-0 z-30 flex h-14 items-center justify-between border-b border-white/10 bg-sidebar/80 px-4 backdrop-blur-xl md:hidden">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-white/10 bg-sidebar/90 backdrop-blur-xl transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0",
      ...
      >
        ...
        <div className="p-3">
          <Link
            ...
            className="flex w-full items-center gap-2 rounded-xl bg-primary px-3 py-2.5 text-sm font-medium text-primary-foreground shadow-[0_0_24px_-4px] shadow-primary/50 hover:opacity-95"
          >
        ...
        <nav className="space-y-0.5 px-3">
          {NAV.map((n) => (
            <Link
              ...
              className={cn(
                "flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-sidebar-foreground/80 transition hover:bg-white/5",
                path.startsWith(n.to) && "bg-white/8 font-medium text-foreground",
              )}
            >
        ...
        <div className="mt-4 flex-1 overflow-hidden px-3">
          ...
          <div className="mt-2 max-h-[38vh] space-y-0.5 overflow-y-auto">
            {chats.length === 0 ? (
              ...
              chats.map((c) => (
                <div key={c.id} className="group relative rounded-lg hover:bg-white/5">
                  {editingChatId === c.id ? (
                    <input
                      autoFocus
                      value={renameTitle}
                  ...
                  {menuOpenId === c.id && (
                    <>
                      ...
                      <div className="absolute right-0 top-8 z-40 w-40 rounded-xl border border-white/10 bg-popover p-1 shadow-xl">
                        <button
        ...
        <div className="border-t border-white/10 p-3">
          ...
          <div className="mt-2 flex items-center gap-3 rounded-lg px-2 py-2">
            <div className="grid size-9 place-items-center rounded-full bg-primary/20 text-sm font-semibold text-primary">
              {initials}
            </div>
    ...
    <Modal open={!!chat} onClose={onClose} title="Add to project" size="md">
      <div className="space-y-2">
        {projects.map((p) => (
          <button
            ...
            className={cn(
              ...
              selected === p.id ? "border-primary bg-primary/10" : "border-white/10 hover:bg-white/5",
            )}
          >
        ...
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl border border-dashed border-white/15 p-3 text-sm text-muted-foreground hover:bg-white/5"
        >
          <Plus className="size-4" /> New project
        </button>
      </div>
      <div className="mt-4 flex justify-end gap-2">
        <button onClick={onClose} className="rounded-lg border border-white/10 px-4 py-2 text-sm">
          Cancel
        </button>
        <button
