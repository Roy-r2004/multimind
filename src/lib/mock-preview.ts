...
const PRICING_LESSON: ApiLessonDetail = {
  ...
  verdict_reason:
    "Synthesized conservative financial framing from the model council; weighted margin protection.",
  strategy: "Debate",
  comparison: {
    overview:
    ...
    user_position_summary:
      "Penetration pricing wins when shelf space and partner momentum are the scarce assets.",
    model_position_summary:
      "Premium pricing protects margin and avoids a painful future price increase.",
    agreements: [
      {
        topic: "Brand matters",
        detail: "Both sides want ACME perceived as quality, not a budget tool.",
      },
      {
        topic: "Retail partners are key",
        detail: "Neither disputes that partner shelf is the primary Q1 channel.",
      },
    ],
    disagreements: [
      {
        ...
        analysis:
          "Disagreement is really about time horizon — 90-day land grab vs 12-month margin curve.",
      },
      {
    ...
    assumptions: {
      ...
      model: [
        "Market tolerates premium without trial friction",
    ...
    lesson: {
      ...
      when_user_might_be_right:
        "Signed channel, narrow competitive window, measurable sell-through KPIs.",
...
export const MOCK_LESSON_BY_ID: Record<string, ApiLessonDetail> = {
  ...
  "mock-hiring": {
    ...
    disagreement_reason:
      "The verdict treated headcount as progress. We're still pivoting the roadmap weekly.",
    user_position:
      "Hire one staff engineer who has shipped 0→1. Add juniors only after architecture stabilizes.",
    verdict_model_id: "claude",
    verdict_text:
      "Build a balanced 6-person pod for parallel workstreams across frontend, backend, and infra.",
    verdict_reason:
      "Pick Best strategy favored the model with strongest engineering org design argument.",
    strategy: "Pick Best",
    comparison: {
      ...
      blind_spots: {
        user: ["Bus factor on single staff engineer"],
        model: ["Coordination overhead of six"],
      },
      lesson: {
        ...
        recommended_next_step:
          "Staff engineer trial project for 2 weeks before opening other reqs.",
      },
    },
    error_message: null,
  },
  "mock-gtm": {
    ...
    verdict_text:
      "Own the customer relationship from day one — build direct sales motion immediately.",
    ...
    comparison: {
      ...
      user_position_summary:
        "Partners are a distribution accelerant with a planned direct transition.",
      ...
      evidence: [],
      assumptions: {
        user: ["Partners hit volume targets"],
        model: ["Direct can scale fast enough"],
      },
